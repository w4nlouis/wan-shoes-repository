...
<body>
    <div class="dashboard">
        ...
        <?php if($role == 'admin'): ?>
            <?php if($edit_product): ?>
                ...
                <form method="POST" onsubmit="return validateProduct()">
                    ...
                    <input type="text" id="prod_name" name="name" value="<?php echo $edit_product['name']; ?>" required>
                    <input type="text" id="prod_brand" name="brand" value="<?php echo $edit_product['brand']; ?>" required>
                    <input type="number" id="prod_size" name="size" value="<?php echo $edit_product['size']; ?>" required>
                    <input type="number" step="0.01" id="prod_price" name="price" value="<?php echo $edit_product['price']; ?>" required>
                    <input type="number" id="prod_stock" name="stock" value="<?php echo $edit_product['stock_quantity']; ?>" required>
            ...
            <?php endif; ?>
        <?php else: ?>
            <p style="margin-bottom: 20px; color: #666;">View only mode. Contact admin for product changes.</p>
        <?php endif; ?>
            ...
            <tr>
                ...
                <?php if($role == 'admin'): ?>
                    <td>
                        ...
                        <a href="?delete=<?php echo $row['product_id']; ?>" onclick="return confirmDelete('<?php echo $row['name']; ?>')">Delete</a>
                    </td>
                <?php endif; ?>
            </tr>
            <?php endwhile; ?>
        ...
        <?php else: ?>
            <?php if(isset($_GET['view']) && $_GET['view'] == 'customer'): ?>
                <a href="../customer/dashboard.php" class="back-link">← Back to Customer Dashboard</a>
            <?php else: ?>
